module.exports = (RED) => {
    ...
    if (this.blynkClient) {
      ...
      this.blynkClient.on('status-connecting', () => { // eslint-disable-line no-shadow
        node.status({
          fill: 'yellow',
          shape: 'dot',
          text: RED._('blynk-iot-out-lcd.status.connecting') + node.pin,
        });
      });
      this.blynkClient.on('status-connected', () => {
        node.status({
          fill: 'green',
          shape: 'dot',
          text: RED._('blynk-iot-out-lcd.status.connected-fixed') + node.pin,
        });
      });
      this.blynkClient.on('status-error', () => {
        node.status({
          fill: 'red',
          shape: 'ring',
          text: 'blynk-iot-out-lcd.status.error',
        });
      });
      this.blynkClient.on('status-disconnnected', () => {
        node.status({
          fill: 'red',
          shape: 'ring',
          text: 'blynk-iot-out-lcd.status.disconnected',
        });
      });
      this.blynkClient.on('status-disabled', () => {
        node.status({
          fill: 'red',
          shape: 'dot',
          text: 'blynk-iot-out-lcd.status.disabled',
        });
      });
    } else {
      this.error(RED._('blynk-iot-out-lcd.errors.missing-conf'));
    }
    this.on('input', (msg) => {
      ...
      if (msg.hasOwnProperty('payload')) {
        if (node.pin < 0 || node.pin > 255) {
          node.warn(RED._('blynk-iot-out-lcd.warn.pin-value'));
          return;
        }
        ...
        if (!msg.hasOwnProperty('text') && !msg.hasOwnProperty('text1') && !msg.hasOwnProperty('clear') && msg.hasOwnProperty('payload')) {
          ...
          if (payload !== '' && payload.length > 1) {
            node.warn(RED._('blynk-iot-out-lcd.warn.payload'));
          }
        }
      }
    });
  ...
  RED.nodes.registerType('blynk-iot-out-lcd', BlynkOutLCDNode);
};
