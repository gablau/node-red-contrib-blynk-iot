module.exports = (RED) => {
  function BlynkStyleBtnNode(n) {
    ...
    this.connected_label = RED._('blynk-iot-style-btn.status.connected-fixed') + this.pin;
    ...
    if (this.blynkClient) {
      ...
      this.blynkClient.on('opened', (n) => { // eslint-disable-line no-shadow
        node.status({
          fill: 'yellow',
          shape: 'dot',
          text: RED._('blynk-iot-style-btn.status.connecting') + n,
        });
      });
      ...
      this.blynkClient.on('error', () => {
        node.status({
          fill: 'red',
          shape: 'ring',
          text: 'blynk-iot-style-btn.status.error',
        });
      });
      this.blynkClient.on('closed', () => {
        node.status({
          fill: 'red',
          shape: 'ring',
          text: 'blynk-iot-style-btn.status.disconnected',
        });
      });
      this.blynkClient.on('disabled', () => {
        node.status({
          fill: 'red',
          shape: 'dot',
          text: 'blynk-iot-style-btn.status.disabled',
        });
      });
    } else {
      this.error(RED._('blynk-iot-style-btn.errors.missing-conf'));
    }
    ...
    this.on('input', function input(msg) {
      ...
      if (msg.hasOwnProperty('payload') && msg.hasOwnProperty('topic')) {
        ...
        if (prop !== '1') { // fixed multiple property
          if (prop === '') {
            node.warn(RED._('blynk-iot-style-btn.warn.bycode'));
            return;
          }
  ...
  RED.nodes.registerType('blynk-iot-style-btn', BlynkStyleBtnNode);
};
