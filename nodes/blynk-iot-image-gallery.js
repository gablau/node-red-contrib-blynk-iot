...
module.exports = (RED) => {
  function BlynkImageGalleryNode(n) {
    ...
    this.connected_label = RED._('blynk-iot-image-gallery.status.connected-fixed') + this.pin;
    ...
    if (this.blynkClient) {
      ...
      this.blynkClient.on('status-disabled', () => {
        node.status({
          fill: 'red',
          shape: 'dot',
          text: 'blynk-iot-image-gallery.status.disabled',
        });
      });
    } else {
      this.error(RED._('blynk-iot-image-gallery.errors.missing-conf'));
    }
  ...
  RED.nodes.registerType('blynk-iot-image-gallery', BlynkImageGalleryNode);
};
