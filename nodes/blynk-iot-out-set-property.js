    ...
    if (this.blynkClient) {
      ...
      this.blynkClient.on('opened', (n) => { // eslint-disable-line no-shadow
        node.status({
          fill: 'yellow',
          shape: 'dot',
          text: RED._('blynk-iot-out-set-property.status.connecting') + n,
        });
      });
      ...
      this.blynkClient.on('error', () => {
        node.status({
          fill: 'red',
          shape: 'ring',
          text: 'blynk-iot-out-set-property.status.error',
        });
      });
      this.blynkClient.on('closed', () => {
        node.status({
          fill: 'red',
          shape: 'ring',
          text: 'blynk-iot-out-set-property.status.disconnected',
        });
      });
      this.blynkClient.on('disabled', () => {
        node.status({
          fill: 'red',
          shape: 'dot',
          text: 'blynk-iot-out-set-property.status.disabled',
        });
      });
        ...
        if (msg.hasOwnProperty('payload')) {
          ...
          if (node.pinmode == 1) { // eslint-disable-line eqeqeq
            ...
            if (msg.pin < 0 || msg.pin > 255) {
              node.warn(RED._('blynk-iot-out-set-property.warn.pin-value'));
              return;
            }
            pin = msg.pin;
          }
          ...
          if (prop !== 'bycode') { // single property
            if (prop === '') {
              node.warn(RED._('blynk-iot-out-set-property.warn.bycode'));
              return;
            }
      ...
      this.error(RED._('blynk-iot-out-set-property.errors.missing-conf'));
    }
  }

  RED.nodes.registerType('blynk-iot-out-set-property', BlynkOutSetPropertyNode);
};
