...
module.exports = (RED) => {
  function BlynkZergbaNode(n) {
    ...
    this.connected_label = RED._('blynk-iot-zergba.status.connected-fixed') + this.pin;
    ...
    if (this.blynkClient) {
      ...
      this.blynkClient.on('opened', (n) => { // eslint-disable-line no-shadow
        node.status({
          fill: 'yellow',
          shape: 'dot',
          text: RED._('blynk-iot-zergba.status.connecting') + n,
        });
      });
      ...
      this.blynkClient.on('error', () => {
        node.status({
          fill: 'red',
          shape: 'ring',
          text: 'blynk-iot-zergba.status.error',
        });
      });
      this.blynkClient.on('closed', () => {
        node.status({
          fill: 'red',
          shape: 'ring',
          text: 'blynk-iot-zergba.status.disconnected',
        });
      });
      this.blynkClient.on('disabled', () => {
        node.status({
          fill: 'red',
          shape: 'dot',
          text: 'blynk-iot-zergba.status.disabled',
        });
      });
    } else {
      this.error(RED._('blynk-iot-zergba.errors.missing-conf'));
    }
  ...
  RED.nodes.registerType('blynk-iot-zergba', BlynkZergbaNode);
};
